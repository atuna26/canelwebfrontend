...
const EbatliProducts = (props) => {
  const product = useSelector((state) => state.product);
  const dispatch = useDispatch();

  const getProduct = async () => {
    const response = await fetch ("https://canelwebbackend.onrender.com/product/ebatli-product-list", {
      method: "GET",
      headers: {
  ...
  return (
    <div className='flex flex-col gap-10 w-full'> 
        <div className='h-[150px] w-full bg-primary flex justify-center items-center'>
            <h2 className='text-white text-3xl font-light font-abel'> <span className="border-t-2 border-l-2 pl-1">Ebatlı ve Fayans</span><span className="border-b-2 border-r-2 pr-1"> Mermerler</span> </h2>
        </div>
        <div className={`flex flex-row gap-5 flex-wrap w-full align-center md:justify-start justify-center`}>
          {product.map((item) => (
