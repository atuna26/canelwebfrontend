...
const ReferenceList = () => {
    ...
    const deleteReference = async (id) => {
        const response = await fetch(`https://canelwebbackend.onrender.com/reference/delete-reference/${id}`, {
            method: "DELETE",
            headers: {
        ...
        <table className="w-full bg-accent text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            ...
            <tr key={reference._id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
              <td className="p-4">
                <img src={`https://canelwebbackend.onrender.com/uploads/${reference.imagePath}`}  alt="" className="w-[50px] h-[50px]" />
              </td>
              <th
                scope="row"
