...
const NewReferences = () => {
    ...
    const handleSubmit = async (values, {resetForm}) =>{
        ...
        const response = await fetch("https://canelwebbackend.onrender.com/reference/new-reference", {
            method: "POST",
            headers: {
  ...
  return (
    <section className={`flex flex-col bg-secondary w-full justify-center shadow-2xl gap-3 px-20 ${styles.paddingY} `}>
      <h2 className='text-white text-4xl font-light text-center'>Referans</h2>
      <p className='text-white'>— Referans - Referans Ekle</p>
            ...
            {({isSubmitting,setFieldValue})=>(
                <Form action='' className='grid grid-cols-2 w-full mt-2 gap-5'>
                    <div className='flex flex-row gap-2 w-full'>
                        <label htmlFor="title" className='text-white font-light'>Referans Adı:</label>
                        <Field type="text" name="title" className='rounded-md p-2 w-full' />
                        <ErrorMessage name="title" component="div" className="text-red-500 text-sm" />
                    </div>
                    <div className='flex flex-row gap-2 w-full'>
                        <label htmlFor="image" className='text-white font-light block mb-2 text-sm'>Referans Resmi:</label>
                    ...
                    <button type='submit' className='bg-primary text-white rounded-md p-2 w-full col-span-2' disabled={isSubmitting}>
                        {isSubmitting ? "Kaydediliyor..." : "Referansı Kaydet"}
                    </button>
                </Form>
            )}
