...
const Fair = () => {
  ...
  const getEvent = async () => {
    const response = await fetch("https://canelwebbackend.onrender.com/event/event-list", {
      method: "GET",
      headers: {
  ...
  const getReferences = async () => {
      const response = await fetch(`https://canelwebbackend.onrender.com/reference/reference-list`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <section className={`flex flex-col col-span-2 bg-white w-full justify-center shadow-2xl gap-3 px-20 ${styles.paddingY}`}>
      ...
      <div className={`md:grid md:grid-cols-5 flex flex-col  w-full gap-3 sm:pt-10 pt-3`}>
        ...
        <div className='col-span-3 flex  flex-row justify-evenly w-full'>
          <img src={canelizmir2024} className='sm:w-[320px] w-[200px]' loading="lazy" alt="" />
          <img src={canelizmir22} className='sm:w-[320px] w-[200px]' loading="lazy" alt="" />
        </div>
      </div>
      ...
      <div className={`flex flex-row w-full justify-start items-center gap-3 overflow-hidden`}>
          ...
          <img key={reference._id} src={`https://canelwebbackend.onrender.com/uploads/${reference.imagePath}`} loading='lazy' alt={reference.title}
